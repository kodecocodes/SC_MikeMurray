...
class OldAndBustedAnimal {
    ...
    var isInvertebrate: Bool
    
    init() {
        self.isInvertebrate = true
    }
 ...
 */
protocol Hero {
    func assembleWith(heroes: [Self])
    func saveTheWorld()
}

final class Avenger: Hero {
    func assembleWith(heroes: [Avenger]) {
        // Avengers assemble!
    }
...
/*:
 We now get the assembleWith function for free in our Avenger class! A few things to note here.
...
/*:
 We are making use of associated types, which in a nutshell enables us to use generics in our protocols. An associatedtype is essentially a typealias that we provide in any objects which adopt the CRUD protocol, let's take a look at an implementation of our protocol.
 */
